...
ENV_RHO = 0.01 # randomness in the environment
...
def train_q_agent(environment: GridWorld, agent: QAgent, trials=1000, max_steps_per_episode=100):
    ...
    for _ in range(trials):
        ...
        while step < max_steps_per_episode:
            ...
            action = agent.get_action(state, explore=True) # Epsilon greedy action
            next_state, reward, done = environment.step(action)
            cumulative_reward += reward
            agent.update(state, reward, next_state, action) # Update Q table
            state = next_state
            step += 1
            # Check if episode is done
            if state in environment.terminal_states:
                # If agent is in terminal state, set done to True
                done = True
            if done:
                break
...
if __name__ == '__main__':
    ...
    agentQ.visualize_q_values(title="q_agent_initial")
    ...
    # visualize Q values
    agentQ.visualize_q_values(title="Best Q Values")